...
- Если ваш crate имеет `features`, проведите испытания для всех возможных комбинаций доступных `features`
...
## Проблемы тестирования:
 ...
 - Нет доступа к коду для его тестирования (к примеру код спаггети, где весь функционал вместе). Нужно сразу учитывать как протестировать этот функционал
...
Так как модульные тесты идут в тех же файлах что и основной код, вы будете использовать `#[cfg(test)]` чтобы указать, что они не должны быть включены в скомпилированный исполняемый файл программы.

Модульное тестирование проверяет отдельные функции или «единицы» кода.

Модульные тесты выполняют две основные функции:

- Проверяют, что отдельные части программы (модули, функции, классы) работают корректно – то есть соответствуют ожидаемому поведению.

- Защищают от регрессий – предотвращают случайное изменение существующей функциональности при внесении новых изменений или исправлений.

## Integration tests

Интеграционное тестирование. (файл `lib.rs` должен быть)
...
`afl` позволяет запускать AFL (американский fuzzy lop) на коде, написанном на Rust.
